...
public class Song extends AudioContent implements Comparable<Song> // implement the Comparable interface
{
			...
			String composer, Song.Genre genre, String lyrics)
	{
		...
		super(title, year, id, type, audioFile, length); // calling parent constructor to initialize instance variables
		this.artist=artist;
	...
	public void printInfo()
	{
		...
		System.out.println("Artist: "+this.artist+" Composer: "+this.composer +" Genre: "+this.genre); // printing out artist, composer and genre name
	}
	
	// Play the song by setting the audioFile to the lyrics string and then calling the play() method of the superclass
	public void play()
	{
		super.setAudioFile(lyrics); //set audioFile instance variable with value of lyrics
	...
	{ 
		if(other instanceof AudioBook){
			...
			Song otherSong=(Song)other; //Creating a Song object called by casting Object other to Song
		if(super.equals(otherSong) && (this.composer.equals(otherSong.composer)&&(this.artist.equals(otherSong.artist)))){
			return true;
		}
	...
	{
		Song otherSong=(Song)other; //Creating a Song object called by casting Object other to Song
		return this.getTitle().compareTo(otherSong.getTitle()); // compares the titles of two song objects returns either true or false
